...
from sqlalchemy import String, Text, Integer, ForeignKey, DateTime, func, JSON, Boolean, Index
...
class SnapshotInstance(Base):
    ...
    data_path: Mapped[Optional[str]] = mapped_column(String(500), nullable=True)  # 本地快照文件路径
    created_at: Mapped[datetime] = mapped_column(DateTime, server_default=func.now())

    __table_args__ = (
        Index("idx_snapshot_comm", "snapshot_id", "communication_id"),
    )
...
class EnvironmentData(Base):
    ...
    data_value: Mapped[Optional[dict]] = mapped_column(JSON, nullable=True) # 兼容老数据，新数据可能存文件
    has_file_data: Mapped[bool] = mapped_column(Boolean, default=False) # 标记数据是否在实例关联的文件中
    checksum: Mapped[Optional[str]] = mapped_column(String(64), nullable=True)
    created_at: Mapped[datetime] = mapped_column(DateTime, server_default=func.now())

    __table_args__ = (
        Index("idx_instance_item", "snapshot_instance_id", "check_item_id"),
        Index("idx_item_created", "check_item_id", "created_at"),
    )
